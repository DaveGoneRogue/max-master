        ...
        \begin{tikzpicture}
            \foreach \z in {1}{  % Controles whether the axes are inverted or not, 1 = yes, anything else = no
                ...
                \foreach \y in {-1,...,3}{
                    \ifnum\x=-1
                        \ifnum\y=3
                            \draw[->, purple] (\x,\y) -- (\x,\y+\AlphaONE) node[pos=0.3, right, xshift=-0.9mm] {\scriptsize $\alpha$ = \AlphaONE};  % Line indicating the shift amount
                        \else
                        \draw[->, purple] (\x,\y) -- (\x,\y+\AlphaONE);  % Line indicating the shift amount
                        \fi
                    \else
                    \draw[purple] (\x,\y) -- (\x,\y+\AlphaONE);  % Line indicating the shift amount
                    \fi
