...
\textcolor{green}{kommentar: a,b ? droppe 0,1} %! Kommentar
...
\begin{proof}
    ...
    \end{figure}
    %Since $C([0,1])$ is dense in $L^2([0,1])$ from \cref{lem:c_dense_L2}, it will then follow that $\Cper([0,1])$ is dense in $L^2([0,1])$ if we can show that $\Cper([0,1])$ is dense in $C([0,1])$.
    %If we can show that $\Cper([0,1])$ is dense in $C([0,1])$, it will follow that $\Cper([0,1])$ is dense in $L^2([0,1])$, since we know from \cref{lem:c_dense_L2} that $C([0,1])$ is dense in $L^2([0,1])$. 
    ...
    \begin{equation*} % kan lage formel for g på ax+b formel: dvs $dy/dx \cdot x + b$ ELLER  $ -g(1-\delta) / \delta \cdot x + g(1-\delta)/ \delta $ <- dette er høyre side.
        g(t) = 
        \begin{cases} 0, &  t=0,\\  
