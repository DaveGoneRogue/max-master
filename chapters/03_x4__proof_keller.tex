...
For this proof, we must redefine our working definition of the unit cube from $\brac{0,1}$ into the half-open interval $[0,1)$. This is because the proof will heavily rely on geometric arguments, and we make the proof simpler if we ensure that two distinct cubes have a disjoint interior rather than an interior with measure zero. We must also redefine our working definition to use the concepts of non-overlapping and cover. Note that for a tiling set to be non-overlapping, it now means that the intersection of two distinct cubes must be empty.
