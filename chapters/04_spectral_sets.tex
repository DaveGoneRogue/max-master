...
\colorbox{BurntOrange}{Transition sentence here? Do not quite feel like it begins meaningfully, or am I the only one}
...
Due to the restrictions imposed by the orthogonality requirement, there is limited flexibility in constructing spectral sets, making them highly rigid structures. As a consequence, not many sets $\Omega$ have the property of being spectral. However, once we have one spectrum for a spectral set, observe that the spectrum is shift-invariant.
...
\begin{proof}
    ...
    \end{align*}
    This shows that $E(\Lambda_\alpha)$ is orthogonal since \SigridChange{$E($}$\Lambda$\SigridChange{$)$} is orthogonal. Now, to show completeness assume that $f\in L^2(\Omega)$ satisfies $\braaMed{f,e_{\lambda + \alpha}} = 0$ for all $\lambda\in \Lambda_\alpha$, and observe that
    \begin{align*}
...
%! The following commented files should be deleted and better merged with the uncommented section bellow     
