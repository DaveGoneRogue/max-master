...
\begin{lemma}\label{lem:zero_set_jp_1_5}
    \sout{Let $\Omega=I^d$.} The zero-set for the function
    \begin{equation*}
        F_{\Omega} (z):= \int_\Omega e^{2 \pi i \braa{z,t}} dt,
    \end{equation*}
    \SigridChange{when $\Omega=I^d$} is the set
    \begin{equation*}
...
\SigridComment{To big to be a remark? Also, is the opposite statement correctly phrased?}
...
\begin{proof}[Proof of \cref{thrm:construction_spectra}]
    ...
    \end{align*}
    \mycomment{  %! Block comment, this is now in \intertext to get the alignment right. \phantom{text} to create invisible eq. w. alignment did not work
    ...
    \mycomment{  %! Block comment %* Old variant
...
\begin{proof}
    ...
    Furthermore, observe $\Lambda$ is, in fact, a subset given by \labelcref{eq:2d_all_shift_class_1}, with $\lambda_1 = m + \alpha$ for $\alpha = 0$ and $\lambda_2 = n+\beta_m$. That is,
    %! ABOVE
    \SigridComment{Doubblechecked. Sigrid actually wants to delete the $\alpha$ values, which means she thinks it is wrong — is it wrong?}
    ...
    \end{equation*}
    %! HERE
    \SigridComment{Doubblechecked. Sigrid actually wants to delete the $\alpha$ values, which most likly means she thinks it is wrong — is it wrong?}
