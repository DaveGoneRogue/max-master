...
\begin{remark}\label{rem:zero_set_orthogonal}  %* One way implication, =>, which took a long time to find out. The equivalent result is Keller's theorem.
...
\SigridComment{Consider changing the order, as remark 2.18 with lemma 4.7 gives now remark 4.10. Then we can create an equivalent result which is a rephrasing of remark 4.10, and that is Kellers spectral version, now lemma 4.8}

\SigridComment{also consider changing the wording in Spectral version so that the zero set becomes more apparent. This can either be done before, as to better build up remark 4.10. Or change the order, and use it in the introduction/build up of the lemma.}
