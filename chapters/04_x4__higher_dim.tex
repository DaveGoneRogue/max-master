...
\SigridComment{Neinei}
%! Max comment, de har faktisk splittet det opp med vilje her!!!!! Faen heller, se på ny struktur her på nytt. 

    \begin{equation*}
        \Zstroke_{\Omega} = \braqMed{ z = \brac{z_1,\dots,z_d}\in \C^d \setminus \braqMed{0} : \exists \space j \in \braqMed{1,\dots, d} \text{ such that } z_j \in  \intnozero}.
    \end{equation*}
\end{lemma}
...
\begin{proof}
    ...
    \begin{equation*}\label{eq:zeroset_dim_2}
        \Zstroke_{I^2} = \braqMed{z = \brac{z_1,z_2} \in \C^2\setminus \braqMed{0} : \exists \space j \in \braq{1,2} \text{ such that } z_j \in \intnozero}.
    \end{equation*}
