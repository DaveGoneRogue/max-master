...
\SigridComment{Make some note of the use of distinct elements and what that corresponds to}

\textcolor{orange}{  %! Here
Moreover, using \cref{lem:zero_set_orthoganl_general} in combination with $\Zstroke_{I^d}$ from \cref{lem:zero_set_jp_1_5}, we can formulate a result that closely resembles Keller's \namecref{thrm:keller_tiling} for tiling sets (\cref{thrm:keller_tiling}) \cite{lagariasOrthonormalBasesExponentials2000,iosevichSpectralTilingProperties1998}.
%! can be used to rewrite the introduction sentence on line 51 above.
% aptly named \emph{the spectral version of Keller's theorem} \cite{iosevichSpectralTilingProperties1998}, which we have previously introduced as our \cref{lem:zero_set_jp_1_5} with \cref{rem:zero_set_orthogonal}. They are the same, with the distinction being in BLABLABLA. 
...
\textcolor{orange}{  %! Here
Although the wording is different from Kellers' \namecref{thrm:keller_tiling}, the connection between \cref{lem:zero_set_AiSp} and Kellers' \namecref{thrm:keller_tiling} will become more apparent later in \cref{chap:equivalence}. However, for the interested reader, the connection follows from \cref{rem:key_insight} and \cref{lem:zero_set_jp_1_5}.  ISH. 
%Furthermore, \cref{lem:zero_set_AiSp} also allows us to state the following result for spectral pairs. 
\begin{remark}\label{rem:zero_set_orthogonal}  %* One way implication, =>, which took a long time to find out. NOT the same as "Spectral Keller"
    If given a spectral pair $(I^d,\Lambda)$, it follows from \cref{def:spectral_set,lem:zero_set_AiSp} that the spectral pair property is equivalent to the set $E(\Lambda)$ is complete in $L^2(\Omega)$, and that the non-zero elements of $\Lambda - \Lambda$ being contained in the zero-set of $F_{I^d}$, meaning 
    \begin{equation*}%\label{eq:zero_set_inclusion}
        \Lambda - \Lambda \subseteq \Zstroke_{I^d} \cup \braq{0}.
    \end{equation*}
    \SigridComment{This equivalence of non-zero is taken from Jorgensen and Pedersen, it doesn't match the rest of the thesis. }
\end{remark}
} %! Here
...
\SigridComment{Ish, needs to be rewritten.}
\begin{remark}
