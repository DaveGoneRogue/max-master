...
\begin{proof} %! [Proof of  \cref{lem:zero_set_jp_1_5}] % Hvis proof med navn
    ...
    \end{align*}
    Note that if any $z_j = 0$, we interpret the corresponding factor in the product as $1$. Furthermore, assume that $z\neq 0$ as this implies that at least one factor $z_j\neq 0$. In this case, observe that the equation is zero whenever at least one element $z_j\in \intnozero$ using that the exponential equals one in this case. Thus, the zero set for $F_{I^d}$ is $\Zstroke_{I^d}$ which completes the proof. %* First sentence: In other words, reducing the total number of factors in the product.
...
\begin{example} %! Opp til deg, derfor vi ikke har tatt det vekk
...
\begin{proof}[Proof of  \cref{thrm:construction_spectra}] %* Meningen med to mellomrom før \cref her for bedre spacing:)
    ...
    \begin{equation*}
        \int_{\Omega_1}e^{2 \pi i \lambda_1 t_1} \overline{f(t_1,t_2)} dt_1 dt_2 = 0
    \end{equation*}
    for almost every $t_2\in \Omega_2$. This follows from $(\Omega_2,\lambfunc)$ being a spectral pair. Now, since $\lambda_1$ was arbitrarily and $(\Omega_1,\Lambda_1)$ is a spectral pair we have that $f(t_1, t_2)=0$ for almost every $(t_1,t_2) \in \Omega_1 \times \Omega_2$. 
    ...
    \mycomment{  %* Old variant
    If we fix $\lambda_2$, we denote the inner integral by
    \begin{equation*}%\label{eq:inner_eq}
        F(t_2) := \int_{\Omega_1} e^{2 \pi i \lambda_1 t_1} \overline{f(t_1,t_2)} dt_1,
    \end{equation*}
    and note that $F\in L^2(\Omega_2)$. We thus have % This allows us to rewrite our initial inner product into
    \begin{equation*}%\label{eq:outter_eq}
        \braa{e_{\lambda}, f}_{L^2(\Omega_1\times \Omega_2)} = \int_{\Omega_2} e^{2 \pi i \lambda_2 t_2} F(t_2) dt_2 = \braa{e_{\lambda_2}, F}_{L^2(\Omega_2)} = 0, \quad \text{ for all } \lambda_2 \in \Lambda_2 .
    \end{equation*}
...
\SigridComment{1: tykke linjer vs tynne linjer. 2. Piler ved beta verdiene, ikke piler ved beta verdiene}
...
\begin{example}\label{ex:single_shift_vertical}
    ...
    \begin{align}\label{eq:single_shift_vertical}
        ...
        \end{cases}
    \end{align}
    %! VURDER Å LEGGE INN $\Lambda$ inn i: ... resulting spectrum $\Lambda$ for $I^2$...
    Then the resulting spectrum for $I^2$ will be given by \labelcref{eq:construction_spectra} with $\lambfuncNoVar$ now given by \labelcref{eq:single_shift_vertical}. That is,
    \begin{equation*}
...
\begin{example}\label{ex:multiple_shift_vertical}
    ...
    \begin{align}\label{eq:multiple_shift_vertical}
        ...
        \end{cases}
    \end{align}
    Then $\Lambda$ given by \labelcref{eq:construction_spectra} is the resulting spectrum for $I^2$ with $\lambfuncNoVar$ now given by \labelcref{eq:multiple_shift_vertical}. That is,
    \begin{equation*}
        \Lambda = \braq{\brac{\lambda_1, \lambda_2}: \lambda_1\in \Z, \lambda_2 \in \lambfunc}.
    \end{equation*}
...
%! No whitespace in multiple references 
...
%\end{proof}
\begin{remark} %! Denne vil sigrid fjerne
...
\begin{example}
    ...
    %! Is this needed? Better to not mention it? 
\end{example}
...
\begin{proof}
    It follows directly from \cref{lem:beta_shift} and \cref{rem:switch_lambda} that both classes given by \labelcref{eq:2d_all_shift_class_1} and \labelcref{eq:2d_all_shift_class_2} make $\brac{I^2,\Lambda}$ a spectral pair. To show that there are no other possibilities for $\Lambda$, we will make use of the inclusion
    ...
    \end{align*}
    we get that $\brac{\lambda-\lambda'} \notin \Zstroke_{I^2}$ which contradicts \labelcref{eq:inclusion_dim_2}. Thus, $\lambda_1'$ must be an integer for any element $\lambda'\in \Lambda$ since this implies
    ...
    \end{align*}
    and that $\brac{\lambda-\lambda'} \in \Zstroke_{I^2}$. We conclude that %! Sier vel egentlig her at $\lambda_1 = m + \alpha$
    ...
    %* DRITER I HVA LAMBDA_2 og LAMBDA_2' er, det viktige er hva antagelsen gjør med det vi har antatt av at $\lambda\notin \Z$ samt $\lambda_1\in\Z$
    %* og at det tvinger $\lambda_1'$ til å også være et heltall
    Going forward, take $\lambda, \lambda' \in \Lambda$ such that $\lambda\neq \lambda'$, and assume $\lambda_1 = \lambda'_1$. Then, from \labelcref{eq:inclusion_dim_2} and the fact that we must have  %* fjernet merknaded i antagelsen om at $\lambda_1,\lambda'_1 \in \Z$
    \begin{equation*}
    ...
    Furthermore, observe $\Lambda$ is, in fact, a subset given by \labelcref{eq:2d_all_shift_class_1}, with $\lambda_1 = m + \alpha$ and $\lambda_2 = n+\beta_m$ where $\alpha = 0$. That is,
    ...
    From \cref{lem:onb_direct_subset}, we know that we cannot have an orthonormal basis as a direct subset of another orthonormal basis. Thus we must have equality, and $\Lambda$ must, in this first case, be of the form \labelcref{eq:2d_all_shift_class_1}.
    ...
    \end{equation*}
    where $k \in \Z$, and $m$ is chosen to to be the largest integer so that the shift $\beta_{\lambda_2} \in [0,1)$. Now, observe that $\Lambda$ is a subset given by \labelcref{eq:2d_all_shift_class_2} with $\lambda_1 = m + \beta_n$ and $\lambda_2 = n+\alpha$ where $\alpha = 0$. That is,
