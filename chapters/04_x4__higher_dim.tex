...
\begin{remark}
    ...
    \end{equation*}
    It follows from \cref{lem:zero_set_jp_1_5} that the spectral pair property is equivalent to the non-zero elements of $\Lambda - \Lambda$ being contained in the zero-set of $F_{\Omega}$, meaning 
...
\SigridComment{Sigrid wants to "consider cutting" this example. Reason being: Unclear what it illustrates for the reader}
\begin{example} %! Opp til deg, derfor vi ikke har tatt det vekk
    As we have verified in \labelcref{eq:zero_set_equation}, the zero-set for $F_{I}$ is  $\Zstroke_{I} = \intnozero$. Furthermore, when $\Lambda=\Z$, the non-zero elements of $\Lambda - \Lambda$ is simply $\intnozero$, and clearly
    \begin{equation*}
...
\begin{proof}[Proof of  \cref{thrm:construction_spectra}] %* Meningen med to mellomrom før \cref her for bedre spacing:)
    ...
    \end{align*}
    \SigridComment{how is the wording in this orange highlighted part }\textcolor{orange}{
...
\SigridComment{1: thick lines (figure 4.1) vs. thin lines (figure 4.2). 2. Arrow symbol at the beta values or no arrows at the beta values. 3. Figure caption}
...
\begin{proof}
    ...
    \end{equation*}
    That is, $\Lambda \subset \Zstroke_{I^2} \cup \braq{0}$. This means that at least one of $\lambda_1$ or $\lambda_2$ must be a non-zero integer for every $\lambda \in \Lambda$. %This argument greatly reduces the possibilities. 
    ...
    We conclude that %! Sier vel egentlig her at $\lambda_1 = m + \alpha$
    ...
    Furthermore, observe $\Lambda$ is, in fact, a subset given by \labelcref{eq:2d_all_shift_class_1}, with $\lambda_1 = m + \alpha$ for $\alpha = 0$ and $\lambda_2 = n+\beta_m$. That is, \space \SigridComment{Sigrid wanted to delete the $\alpha$ and $\alpha=0$ parts. Does this work better, or would you say that this is obvious and there is no need to mention it all?}
    ...
    \end{equation*}
    where $k \in \Z$, and $m$ is chosen to be the integer so that the shift $\beta_{\lambda_2} \in [0,1)$. Now, observe that $\Lambda$ is a subset given by \labelcref{eq:2d_all_shift_class_2} with $\lambda_1 = m + \beta_n$ and $\lambda_2 = n+\alpha$ where $\alpha = 0$. That is,
    \space \SigridComment{ same comment about $\alpha$ here}
