...
\SigridComment{Need input on this formulation!!}

Recall \cref{thrm:keller_tiling}, Keller's \namecref{thrm:keller_tiling}. We have an equivalent version, or more specifically, an equivalent restatement of the \namecref{thrm:keller_tiling} which closely relates to \cref{lem:zero_set_AiSp}. The equivalence follows by generalizing the subtraction of two distinct elements in Keller's \namecref{thrm:keller_tiling} into $(\Lambda-\Lambda)\setminus\braq{0}$, and by observing that $\Zstroke_{I^d}$ from \cref{lem:zero_set_jp_1_5} captures the rest of the requirements.

Recall \cref{thrm:keller_tiling}, Keller's \namecref{thrm:keller_tiling}. We have an equivalent version of the \namecref{thrm:keller_tiling} which is similar to \cref{lem:zero_set_AiSp} in the wording. The equivalence follows by generalizing the subtraction of two distinct elements in Keller's \namecref{thrm:keller_tiling} into $(\Lambda-\Lambda)\setminus\braq{0}$, and by observing that $\Zstroke_{I^d}$ from \cref{lem:zero_set_jp_1_5} captures the rest of the requirements.

\begin{theorem}[Keller's theorem alternative]\label{thrm:keller_alternative}
    If $\brac{I^d,\Lambda}$ is a tiling pair, then 
    \begin{equation*}
        \Lambda - \Lambda \subseteq \Zstroke_{I^d} \cup \braq{0}.
    \end{equation*} 
\end{theorem}
When using this alternative version of Keller's \namecref{thrm:keller_tiling}, the connection to the previously mentioned spectral version of Keller's  \namecref{thrm:keller_tiling}, \cref{lem:zero_set_AiSp}, becomes apparent in that Keller's requirement for a cube tiling corresponds to the exponentials being orthogonal for a discrete set $\Lambda$.  % It also allows us to make the following direct connection between the two results. 
...
\SigridComment{Or would you rather have me do the proof again by essentially splicing the remark with the proof above, and then use that as a proof of the theorem?}


Max remark alternatives. 

Therefore, only that $\Lambda$ cannot be a proper subset of either \labelcref{eq:2d_all_shift_class_1_tiling} or \labelcref{eq:2d_all_shift_class_2_tiling} would be left to show after these arguments. 

Therefore, only that $\Lambda$ cannot be a proper subset of either \labelcref{eq:2d_all_shift_class_1_tiling} or \labelcref{eq:2d_all_shift_class_2_tiling} is left to show after these arguments. 

Therefore, through these arguments, we could have skipped ahead and only needed to show that $\Lambda$ cannot be a proper subset of either \labelcref{eq:2d_all_shift_class_1_tiling} or \labelcref{eq:2d_all_shift_class_2_tiling}.

We could have skipped ahead using these arguments and only needed to show that $\Lambda$ cannot be a proper subset of either \labelcref{eq:2d_all_shift_class_1_tiling} or \labelcref{eq:2d_all_shift_class_2_tiling}.

From these arguments, only that $\Lambda$ cannot be a proper subset of either \labelcref{eq:2d_all_shift_class_1_tiling} or \labelcref{eq:2d_all_shift_class_2_tiling} would be left to show.

Using these arguments, we could have skipped ahead and only needed to show that $\Lambda$ cannot be a proper subset of either \labelcref{eq:2d_all_shift_class_1_tiling} or \labelcref{eq:2d_all_shift_class_2_tiling}.


From these arguments, we could have skipped ahead and only needed to show that $\Lambda$ cannot be a proper subset of either \labelcref{eq:2d_all_shift_class_1_tiling} or \labelcref{eq:2d_all_shift_class_2_tiling}.




Since the tiling set from Keller's theorem is also orthogonal according to the \namecref{cor:tiling_pair_implies_orthogonal}, and we in the proof of \cref{thrm:class_all_shift_2d} show that the orthogonality condition makes so $\Lambda$ is a subset of one of the two classes if $\Lambda$ is a spectrum, then we had only to show that it could not be a proper subset.  
