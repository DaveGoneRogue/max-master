...
It is a well-known result from Fourier analysis that $\Lambda = \Z$ is a spectrum for the unit interval $I$. In fact, not only is the system
\begin{equation*}
...
\end{theorem}
\begin{proof}
    We begin with showing orthogonality. Let $e_{n}, e_{m} \in E(\Z)$ with $n \neq m$. %Then, since complex exponential functions are continuous and Rienman integrable on $I$, they are also Lebesgue integrable on $I$. We compute
    ...
    \end{align}
    Now, using the triangle inequality and combining the estimates in \labelcref{eq:estimate_1} and \labelcref{eq:estimate_2}, we get
    \begin{align*}
...
\SigridComment{put the following sentence in a remark?}
One can generalize the above proof to higher dimensions, where $\Lambda = \Z^d$ would be a spectrum for $I^d$. %*Although we will not show this explicitly, we summarise it in the following result.
...
\end{theorem}
However, whereas we will see in \cref{sec:spec_higher_dim} that $I^d$ for $d>1$ has many other spectra, the flexibility for $d=1$ is minimal. % It is easy to classify all spectra, and they are all closely related since they are translations of the same set.
