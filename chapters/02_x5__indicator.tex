...
\begin{proof}
    As a preliminary step to the proof, observe the following equality between the inner product $\braaMed{e_{\lambda},e_{\lambda'} }$ and $F_{\Omega}(z)$, under the assumption that $\lambda,\lambda'\in \Lambda$ are distinct elements, i.e., $\lambda \neq \lambda'$. Equivalently one can assume $z = \lambda - \lambda' \neq0$ and show the equality the other way.  % (since $\lambda - \lambda' = 0$ implies $\lambda = \lambda'$). 
    \begin{align*}
