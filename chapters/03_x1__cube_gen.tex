...
%We will formally define what we mean by a \emph{tiling set} from \cref{thrm:keller_tiling} later (\cref{def:tiling}), but for now, we continue this informal discussion on exotic tilings. 
Keller's \namecref{thrm:keller_tiling} is one of the main aims of the \namecref{chap:tiling} and will later be proven in \cref{sec:proof_keller}. \SigridChangeTwo{Furthermore, we highlight that Keller's \namecref{thrm:keller_tiling} is essential for the classification of tiling sets. However, the \SigridChange{(the proof of the)} two-dimensional case is postponed to \cref{chap:equivalence} due to the connections with spectra which have not yet been covered. }
%* ———————————————————————————— One dimension  ——————————————————
...
Increasing the dimension by one, we get more flexibility. In particular, when $d=2$, we do not necessarily need to have a lattice tiling as the one in \cref{fig:tiling_one}. We can have tilings where we translate single or multiple \emph{columns} of the unit cube, shown in \cref{fig:single_shift_vertical_tiling,fig:multiple_shift_vertical_tiling}; or single or multiple \emph{rows} of the unit cube, shown in \cref{fig:single_shift_horizontal_tiling,fig:multiple_shift_horizontal_tiling}. \SigridChangeTwo{In \cref{chap:equivalence}, when classifying all tiling sets in the two-dimensional case, we will show that \cref{fig:tiling_figures} to some extent fully captures the flexibility one has in dimension two.} We remark that all \cref{fig:single_shift_vertical_tiling,fig:multiple_shift_vertical_tiling,fig:single_shift_horizontal_tiling,fig:multiple_shift_horizontal_tiling} clearly illustrates that all tilings of the unit cube in $\R^2$ \SigridChange{indeed must} satisfy \SigridChange{both} Keller's \namecref{thrm:keller_tiling} and \SigridChangeTwo{Keller's \namecref{conj:keller_tiling}, the latter of which will be of focus for the remainder of this \namecref{sec:aperi_cube}.}
% OLD: \SigridChangeTwo{In \cref{chap:equivalence}, when classifying all tiling sets for $d=2$, we will show that \cref{fig:tiling_figures} to some extent fully captures the flexibility one has in this case.}
\input{figures/figure-tex/tiling_figures.tex}



%* Old Keller continuation
%Equally important is that Keller also conjectured the following in the same paper \cite{kellerUberLuckenloseErfullung1930} where \cref{thrm:keller_tiling} was first stated and proven. 
\SigridChangeTwo{In the same paper \cite{kellerUberLuckenloseErfullung1930} where Keller's \namecref{thrm:keller_tiling}, \cref{thrm:keller_tiling}, was first stated and proven, Keller conjectured the following of equal importance.}
...
If the dimension is two, the squares share an edge; if the dimension is three, they share a square. \cref{fig:tilings_five_six} illustrates this. Furthermore, when $d\geq 3$, it is not necessarily true that \emph{all} cubes shared a whole $(d-1)$-dimensional face with one of its neighbors \cite{perronUeberLueckenloseAusfuellung1940}. It is important to emphasize that the \namecref{conj:keller_tiling} does not support the existence of \SigridChangeTwo{tilings in which \emph{no} cubes share a whole $(d-1)$-dimensional face with one of its neighbors}. Instead, it indicates the opposite — rigidity in the tiling of the space.
