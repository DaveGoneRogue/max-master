...
under the assumption that $\left\{ e_{n} \right\}_{n\in \mathbb{N}}$ \emph{is} an orthonormal basis for $H$.  By \cref{thrm:orthog_proj_formula_and_facts} we know that this series converges unconditionally, meaning that it converges whatever the ordering of the series is. The orthogonality implies that the 'decomposition' is unique, and in some sense one can say that \cref{eq:orthonormal_equivalences_3} \emph{characterises} completeness. %\cref{thrm:orthonormal_equivalences} gives several other equivalent and useful characterisations for complete orthonormal sequences.
