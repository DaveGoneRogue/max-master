...
\textcolor{green}{kommentar: er ikke helt ferdig her }

An equivalent restatement of completeness is the following
\begin{lemma}\label{lem:ONB_alternative_def}
    An orthonormal sequence $\left\{ e_k \right\}$ is complete in a Hilbert space $H$ if and only if $\langle x, e_k \rangle_{H} = 0$ for all $k\in B$ and some $x\in H$, then $x$ is the zero element of $H$.
\end{lemma}

\begin{proof}
    Since $B$ is an orthonormal set and $x$ is orthogonal to $B$, then $x$ is orthogonal to the closure of the linear span of $B$. As $\spn{B}$ is dense in $H$ we have $\spnclos{B} = H$, and the only vector orthogonal to a dense linear subspace is the zero-vector. 
\end{proof}
