...
\begin{definition}
    ...
    \begin{itemize}%[topsep=0pt]
        ...
        \item A \textsc{set} of vectors $\braq{v_i : i \in I}$ is \emph{orthonormal} if it is orthogonal, and each vector $v_i$ satisfies $\|v_i\| = 1$. %unit vector
        \qedhere
    \end{itemize}
\end{definition}
\begin{remark}
    Note that any set of nonzero orthogonal vectors can be turned into an orthonormal set by dividing each vector by its own length. %!Thus, results stated for orthonormal sets also hold for orthogonal sets. The converse is not necessarily true. % LESE hva som står i HEIL?
\end{remark}
%\begin{definition}
...
\end{lemma}
\begin{proof} %! HENGER IKKE SAMMEN, hør lydlogg
    Since $E$ is an orthonormal set and $x$ is orthogonal to $E$, we must have $x$ orthogonal to the closure of the linear span of $E$. As $\spn{E}$ is dense in $H$ we have $\spnclos{B} = H$, and the only vector orthogonal to a dense linear subspace is the zero-vector. 
\end{proof}
...
\end{lemma}
\begin{proof}
    Suppose for contradiction that $F$ is a strict subset of $E$, which is complete in $H$. Let $v_\gamma$ denote a non-zero element such that $v_{\gamma}\in E$ and $v_{\gamma}\notin F$. Then $v_{\gamma}$ is orthogonal to all elements of $F$, then by \cref{lem:ONB_alternative_def} $v_{\gamma}$ must be the zero-element. This is a contradiction, so we must have that $F=E$ if $F$ is to be an orthonormal basis for $H$.
