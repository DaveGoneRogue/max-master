...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item A \textsc{set} of vectors $\braq{x_i : i \in I}$ is \emph{orthonormal} if it is orthogonal, and each vector $x_i$ satisfies $\|x_i\| = 1$. %unit vector
    \end{enumerate}\qedhere
\end{definition}

\begin{remark}
    Note that any set of nonzero orthogonal vectors can be turned into an orthonormal set by dividing each vector by its length. %!Thus, results stated for orthonormal sets also hold for orthogonal sets. The converse is not necessarily true. % LESE hva som står i HEIL?
\end{remark}
